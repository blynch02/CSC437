...
function create(username, password) {
  return CredentialModel.findOne({ username }).then((foundUser) => {
    if (foundUser) {
...
function verify(username, password) {
  return CredentialModel.findOne({ username }).exec().then((foundUser) => {
    if (!foundUser) {
      throw new Error("Invalid username or password");
    }
    return import_bcryptjs.default.compare(password, foundUser.hashedPassword).then((match) => {
      if (!match) {
        throw new Error("Invalid username or password");
      }
